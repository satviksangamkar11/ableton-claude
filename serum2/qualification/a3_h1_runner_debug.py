"""A3 H1 debug runner — captures and dumps complete result artifacts."""
...
def run_boolean_and_dump() -> None:
    """Run BOOLEAN H1 and dump complete artifacts."""
    ...
    # Build BOOLEAN plan only
    plans = build_h1_pilot_plans()
    boolean_plan = plans[0]

    # Get resolved target
    semantic_id = boolean_plan.claim_subject
    resolved_target = resolved_targets_dict[semantic_id]

    print("\n" + "=" * 70)
    print("A3 H1 BOOLEAN PILOT — DEBUG OUTPUT")
    print("=" * 70)
    ...
    print("\n--- EXPERIMENT SPEC ---")
    spec_dict = {
        "experiment_id": boolean_plan.experiment_id,
        "claim_subject": boolean_plan.claim_subject,
        "claim_predicate": boolean_plan.claim_predicate,
        "mutations": [
            {
            ...
            for m in boolean_plan.mutations
        ],
        "prerequisites": boolean_plan.prerequisites,
        "isolation_level": boolean_plan.isolation_level,
    }
    print(json.dumps(spec_dict, indent=2))

    # Execute qualification
    print("\n--- EXECUTING QUALIFICATION ---")
    receipt = qualify_plan(boolean_plan, resolved_target)
    ...
    print("\n" + "=" * 70)
    print("END BOOLEAN AUDIT")
    print("=" * 70 + "\n")


if __name__ == "__main__":
    try:
        run_boolean_and_dump()
        sys.exit(0)
