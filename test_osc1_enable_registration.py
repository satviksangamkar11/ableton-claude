"""Test that OSC1.Enable is properly registered and resolvable."""
import sys
import pickle
...
print("=" * 80)
print("16.5.52.C: OSC1.Enable Semantic Registration Test")
print("=" * 80)
print()

# Step 1: Check registration
print("[STEP 1] Check semantic target registration")
if "OSC1.Enable" in SEMANTIC_TARGETS:
    ref = SEMANTIC_TARGETS["OSC1.Enable"]
    print(f"  [PASS] OSC1.Enable registered")
    print(f"    - Semantic name: {ref.name}")
    print(f"    - Capability key: {ref.capability_key}")
else:
    print(f"  [FAIL] OSC1.Enable NOT registered")
    sys.exit(1)
...
resolved = resolve_semantic_target("OSC1.Enable", contracts)
...
resolved2 = resolve_semantic_target("OSC1.Enable", contracts)
...
if dry_result.accepted:
    print(f"  [PASS] Admission ACCEPTED for OSC1.Enable")
...
print()
print("=" * 80)
print("[PASS] DECISION: OSC1_ENABLE_SEMANTIC_REGISTRATION_VERIFIED")
print("=" * 80)
