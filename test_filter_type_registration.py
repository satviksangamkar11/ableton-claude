"""Test that Filter.Type is properly registered and resolvable."""
import sys
import pickle
...
print("=" * 80)
print("16.5.52.F: Filter.Type Semantic Registration Test")
print("=" * 80)
print()

# Step 1: Check registration
print("[STEP 1] Check semantic target registration")
if "Filter.Type" in SEMANTIC_TARGETS:
    ref = SEMANTIC_TARGETS["Filter.Type"]
    print(f"  [PASS] Filter.Type registered")
    print(f"    - Semantic name: {ref.name}")
    print(f"    - Capability key: {ref.capability_key}")
else:
    print(f"  [FAIL] Filter.Type NOT registered")
    sys.exit(1)
...
if hasattr(resolved, 'ref'):  # ResolvedTarget
    ...
    print(f"    - Mutation value used: {resolved.contract.scope.get('mutation_value_used')}")
...
resolved2 = resolve_semantic_target("Filter.Type", contracts)
...
if dry_result.accepted:
    print(f"  [PASS] Admission ACCEPTED for Filter.Type")
...
print()
print("=" * 80)
print("[PASS] DECISION: FILTER_TYPE_SEMANTIC_REGISTRATION_VERIFIED")
print("=" * 80)
